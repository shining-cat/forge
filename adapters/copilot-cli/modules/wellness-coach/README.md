...
A context-aware wellness coaching module for GitHub Copilot CLI, bundled with Forge as an optional opt-in. Unlike blind timers, it uses work context (task boundaries, calendar, weather) and personal knowledge to suggest breaks at the right moments, in the right tone, with escalation up to refusing to work.
...
- **Multi-terminal aware** — coordinated across all GitHub Copilot CLI instances
...
- Switching focus away from GitHub Copilot CLI without locking
...
```bash
~/.copilot/skills/wellness-coach/scripts/uninstall-monitor.sh
```
...
State is split across two files in `${VAULT_PATH}/_shared/` (legacy: `~/.copilot/` if Forge isn't installed):
...
```bash
~/.copilot/skills/wellness-coach/scripts/wellness-status.sh --state
```
