...
def _read_forge_conf_value(key, conf_path=None):
    ...
    """
    conf_path = Path(conf_path) if conf_path else Path(os.environ.get("COPILOT_HOME", str(Path.home() / ".copilot"))) / "forge.conf"
...
SCREEN_STATE_BINARY = Path(os.environ.get("COPILOT_HOME", str(Path.home() / ".copilot"))) / "bin" / "screen_state"
