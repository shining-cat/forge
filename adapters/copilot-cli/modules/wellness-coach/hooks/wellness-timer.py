...
def is_vault_write(hook_input):
    ...
    conf_path = os.path.join(copilot_dir, "forge.conf")
...
def is_wellness_script(hook_input):
    ...
    Scripts under `$COPILOT_DIR/skills/wellness-coach/scripts/` (notably
    ...
    return "/.copilot/skills/wellness-coach/scripts/" in command
...
def should_defer_for_meeting():
    ...
    """
    copilot_dir = os.environ.get("COPILOT_HOME", os.path.expanduser("~/.copilot"))
    calendar_sh = os.path.join(copilot_dir, "scripts", "forge-calendar.sh")
