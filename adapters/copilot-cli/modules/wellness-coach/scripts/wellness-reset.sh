...
#
# Usage:
#   ! $COPILOT_DIR/skills/wellness-coach/scripts/wellness-reset.sh
...
#
#   ! $COPILOT_DIR/skills/wellness-coach/scripts/wellness-reset.sh --full-reset
...
if [ "$IF_COLD_START" = "true" ]; then
  ...
  GAP_SCRIPT="$COPILOT_DIR/scripts/forge-gap-since-last-signal.sh"
  [ -x "$GAP_SCRIPT" ] || exit 0
...
PYTHONPATH="$COPILOT_DIR/skills/wellness-coach/hooks" python3 -c "
import os
