...
if [ "${1:-}" = "--diagnose" ]; then
    ...
    if [ "$AM_ENABLED" = "true" ]; then
        ...
        hint "Or re-run: $COPILOT_DIR/skills/wellness-coach/scripts/install-monitor.sh"
    fi
    ...
    echo
    echo "Binary"
    BINARY="$COPILOT_DIR/bin/screen_state"
    if [ -x "$BINARY" ]; then
        ...
        if OUT=$("$BINARY" 2>/dev/null); then
            ...
            hint "Recompile: $COPILOT_DIR/skills/wellness-coach/scripts/install-monitor.sh"
        fi
    else
    ...
    echo
    echo "Sampler"
    SAMPLER="$COPILOT_DIR/bin/idle-sampler.py"
    if [ -x "$SAMPLER" ]; then
    ...
    PLIST_LABEL="com.copilot.wellness-idle-sampler"
