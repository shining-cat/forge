...
#
# The script reads $COPILOT_DIR/forge.conf, $COPILOT_DIR/bin/screen_state,
# and writes to $COPILOT_DIR/wellness-idle-log.json — all via Path.home().
...
# Build a sandbox HOME with .copilot/{bin/screen_state, forge.conf} and an
...
EOF
  chmod +x "$home/.copilot/bin/screen_state"
