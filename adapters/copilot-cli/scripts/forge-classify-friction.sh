...
if [ "$q0_generator_staleness" = "true" ]; then
  ...
  if [ "$q2_safe_to_allowlist" = "true" ]; then
    if [ "$q3_glob_subtlety" = "true" ]; then
      ...
      action_sketch="Add a precise Bash/Write/Edit pattern to $COPILOT_DIR/settings.json"
  ...
  if [ "$q7_verbatim" = "true" ]; then
    ...
    action_sketch="Add (or extend) a GitHub Copilot CLI hook to inject the verbatim string"
  else
    pattern="needs_new_pattern"
