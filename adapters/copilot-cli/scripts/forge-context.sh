...
# Called by GitHub Copilot CLI hooks (PostToolUse, PreToolUse, Stop) and skills.
...
# teammate-notice is purely HOME-relative (sentinel + $COPILOT_DIR/settings.json)
...
FORGE_CONF="${FORGE_CONF_OVERRIDE:-$COPILOT_DIR/forge.conf}"
...
FORGE_PATTERN_CATALOG="${FORGE_PATTERN_CATALOG:-$COPILOT_DIR/skills/forge/references/script-replacement-patterns.md}"
...
# Returns 0 (true) if the current GitHub Copilot CLI session owns the active Forge marker,
...
session_owns_forge() {
  ...
  current_session="${COPILOT_SESSION_ID:-}"
...
get_project_dir() {
  ...
  # outside of GitHub Copilot CLI (e.g. manual script runs).
  local sid="${COPILOT_SESSION_ID:-$$}"
...
# panes (which inherit the lead session's cwd) never re-hit GitHub Copilot CLI's
...
# from GitHub Copilot CLI. Write subcommands (set-marker, append-braindump) and
...
case "$SUBCMD_PEEK" in
  set-marker|park|resume|append-friction|pin-friction|archive-friction-entries|harvest-friction|promote-friction|bootstrap-harvest|audit-prose-rules|skill-budgets|framework-budget|bootstrap-classify|resolve-task|friction-tail|weekly-wrap-due|weekly-wrap-line|teammate-notice|draft-invite-line|draft-list|mark-weekly-wrap-done|substrate-check|review-sync|repo-gh|write-checkpoint|new-task|set-task-status|bump-backlog-header|add-recently-shipped|render-backlog-cell|update-backlog-row|vault-rm|run-tests|trust-anchor)
    ...
    # (e.g. from a post-commit hook in a sibling GitHub Copilot CLI window).
  ...
  *)
    ...
    # context does NOT inherit $COPILOT_SESSION_ID from GitHub Copilot CLI's
...
get_checkpoint_age_minutes() {
  ...
  gap="$("$COPILOT_DIR/scripts/forge-gap-since-last-signal.sh" 2>/dev/null || echo 999999999)"
...
do_post_tool() {
  ...
  # Sibling GitHub Copilot CLI windows reading the same marker file exit silently here
...
" "$STDIN_JSON" 2>/dev/null)" || return 0
  ...
  # from GitHub Copilot CLI's wd, not the user's commit wd, so cwd is unreliable.
  ...
  # Defensive subagent guard — GitHub Copilot CLI issue #34692 says PostToolUse
...
do_gate() {
  # Session-isolation gate: don't block commits in sibling GitHub Copilot CLI windows
...
" 2>/dev/null
  ...
  # GitHub Copilot CLI renders nothing on non-zero exit (silent UX failure).
  ...
  elif ! grep -q '^VAULT_GIT_DECLINED=true' "$COPILOT_DIR/forge.conf" 2>/dev/null; then
    echo ""
    echo "--- Your vault (local history) ---"
...
# go unnoticed. GitHub Copilot CLI renders NOTHING when $COPILOT_DIR/statusline.sh exits
...
do_check_statusline_health() {
  local sl="$COPILOT_DIR/statusline.sh"
  [ -x "$sl" ] || return 0

  # Skip the probe if GitHub Copilot CLI isn't configured to use a statusline.
  ...
  # have a statusline.
  local settings="$COPILOT_DIR/settings.json"
  if [ -f "$settings" ]; then
  ...
  if [ "$exit_code" -ne 0 ]; then
    ...
    echo "[!] Statusline script errored (exit $exit_code) — chip will not render in GitHub Copilot CLI."
    echo "    Script: $sl"
...
do_status() {
  ...
  # Makes session-isolation visible — sibling GitHub Copilot CLI windows that didn't run /forge
...
maybe_daily_install_drift_note() {
  ...
  printf '[Keeper] Daily check: your Forge install is %s commit(s) behind upstream. Update when convenient: (cd %s && git pull && ./install.sh --interactive). Details: $COPILOT_DIR/scripts/forge-context.sh check-install' \
    "$behind" "$forge_repo"
}
...
do_check_install_drift() {
  ...
  # Rollback hint: if any backup artifacts exist under $COPILOT_DIR/, surface that
  ...
  if [ -n "$(_rollback_discover)" ]; then
    echo "    Rollback available: $COPILOT_DIR/scripts/forge-context.sh rollback-install list"
  fi
}
...
# leaves behind under $COPILOT_DIR/:
...
Verbs:
  list                       Inventory of backup artifacts under $COPILOT_DIR/ (default)
  restore <path>             Restore a .pre-update / .pre-remove backup over its target
...
#   (1) sentinel $COPILOT_DIR/forge-teammate-notice-shown absent
#   (2) running inside tmux ($TMUX non-empty)
#   (3) teammateMode resolves to panes (auto|tmux) in $COPILOT_DIR/settings.json
# Otherwise prints nothing (exit 0). HOME-relative so tests can override HOME.
do_teammate_notice() {
  local sentinel="$COPILOT_DIR/forge-teammate-notice-shown"
  [ -f "$sentinel" ] && return 0
  [ -z "${TMUX:-}" ] && return 0
  local settings="$COPILOT_DIR/settings.json" mode=""
  if [ -f "$settings" ]; then
...
# Routing through this script avoids GitHub Copilot CLI's overwrite-confirmation
...
# Bash($COPILOT_DIR/scripts/forge-context.sh *), so the marker write completes
# silently.
#
# Forms:
...
do_set_marker() {
  ...
  case "$form" in
    ...
    active)
      ...
      local session="${COPILOT_SESSION_ID:-}"
      local started
...
do_archive_friction_entries() {
  ...
  # in non-TTY contexts like GitHub Copilot CLI's bash tool, which never closes stdin)
...
HINT
      case "$target" in
        ...
        decision)
          ...
          ;;
        feedback)
          echo "       Path: $COPILOT_DIR/projects/.../memory/feedback_<topic>.md" >&2
          ;;
      esac
      cat <<HINT2 >&2
...
do_audit_prose_rules() {
  ...
  local scan_root="${FORGE_AUDIT_SCAN_ROOT:-$COPILOT_DIR/skills/forge}"
...
do_next_meeting() {
  local calendar_sh="$COPILOT_DIR/scripts/forge-calendar.sh"
...
# Routed through forge-context.sh so `$COPILOT_DIR/scripts/forge-context.sh
