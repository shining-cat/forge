#!/usr/bin/env bash
# forge-permission-lint — anti-pattern linter for $COPILOT_DIR/settings.json
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "  Lints $COPILOT_DIR/settings.json (or --file path) for known anti-patterns."
...
# Single-* doesn't cross / in GitHub Copilot CLI's matcher, so the pattern silently never matches.
check1() {
  local patterns
