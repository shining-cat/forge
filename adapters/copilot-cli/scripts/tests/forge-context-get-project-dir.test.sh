...
EOF
  ...
  export COPILOT_SESSION_ID="get-project-dir-test-$$-$RANDOM"
  WARN_MARKER="/tmp/forge-context-warned-${COPILOT_SESSION_ID}"
...
teardown() {
  rm -rf "$TMP"
  rm -f "$WARN_MARKER"
  unset FORGE_CONF_OVERRIDE COPILOT_SESSION_ID TMP TMP_CONF WARN_MARKER
}
