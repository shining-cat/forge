...
mkhome(){ TMP=$(mktemp -d); mkdir -p "$TMP/.copilot"; [ -n "${1:-}" ] && printf '{\n  "teammateMode": "%s"\n}\n' "$1" > "$TMP/.copilot/settings.json"; }
...
mkhome auto; out=$(run_intmux); echo "$out" | grep -q 'split-panes' && [ -f "$TMP/.copilot/forge-teammate-notice-shown" ] && { echo "✓ open→notice+sentinel"; PASS=$((PASS+1)); } || { echo "✗ open"; FAIL=$((FAIL+1)); }
