#!/usr/bin/env bash
# forge-cost-snapshot.sh — context-economy snapshot for the current GitHub Copilot CLI session
#
# Reads the session JSONL ($COPILOT_DIR/projects/<machine-slug>/<session_id>.jsonl)
...
#   forge-cost-snapshot.sh --session ID  Override session (default: $COPILOT_SESSION_ID)
...
# Resolve JSONL path: $COPILOT_DIR/projects/<machine-slug>/<session_id>.jsonl
...
JSONL_PATH="$COPILOT_DIR/projects/${PROJECT_SLUG}/${SESSION_ID}.jsonl"
...
[ -z "$SESSION_ID" ] && emit_error "no-session-id (COPILOT_SESSION_ID unset)"
