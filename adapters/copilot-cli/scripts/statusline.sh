...
if [ -n "$MARKER" ] && [ -f "$MARKER" ]; then
  ...
  if [ -z "$marker_value" ]; then
    ...
    # COPILOT_SESSION_ID is NOT inherited by the statusline subprocess, so
    ...
    FORGE_STATUS=$(echo "$input" | "$COPILOT_DIR/scripts/forge-context.sh" status 2>/dev/null)
  fi
fi

# Wellness chips — soft-fails to empty if wellness-coach not installed or vault unreachable
WELLNESS_STATUS=""
WELLNESS_SCRIPT="$COPILOT_DIR/skills/wellness-coach/scripts/wellness-status.sh"
