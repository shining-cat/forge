...
**Never** `.copilot/plans/` or `docs/plans/` (per the vault-native plan storage convention, enforced by `forge-vault-plan-guard.sh`).
...
- **Plans go to the vault, not `.copilot/plans/`.**
...
| "Just write the plan to .copilot/plans/, that's the default" | Vault-only. `.copilot/plans/` is invisible to the project's knowledge. |
