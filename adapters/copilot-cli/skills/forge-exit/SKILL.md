...
**Why this MUST run first:** if the wellness coach is already on strike when `/forge-exit` fires, the subsequent Edit/Write tool calls (final checkpoint, marker deactivation) would be blocked. The wellness-coach scripts directory is exempt from strike (see wellness-coach SKILL.md "Strike Conversation" — Bash invocations under `$COPILOT_DIR/skills/wellness-coach/scripts/` are always reachable), so this call always succeeds — clearing the strike before any other step needs to fire.
