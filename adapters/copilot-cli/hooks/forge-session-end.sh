...
#
# Always-on cleanup that runs whenever a GitHub Copilot CLI session terminates
...
#      cleans up. Sibling GitHub Copilot CLI windows that didn't enter Forge
...
#
# Always exits 0 — SessionEnd is observe-only per GitHub Copilot CLI hook
# spec; can't block or fail-loud.
#
# Empirical risk (2026-05-12, when shipped): GitHub Copilot CLI's SessionEnd
# stdin payload may or may not include `session_id`. If env var
# COPILOT_SESSION_ID isn't inherited AND stdin lacks session_id,
