...
setup_env() {
  TMP_HOME="$(mktemp -d)"
  TMP_VAULT="$(mktemp -d)"
  mkdir -p "$TMP_HOME/.copilot" "$TMP_VAULT/_shared"
  echo "VAULT_PATH=$TMP_VAULT" > "$TMP_HOME/.copilot/forge.conf"
  local marker="$1"
  if [ -n "$marker" ]; then
...
# GitHub Copilot CLI populates `agent_id` on hook input only for subagent contexts.
