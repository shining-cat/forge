...
- Write checkpoints at natural pause points (task done, topic shift, before long operations)
  - **Default: subagent dispatch.** Spawn a `forge-keeper` subagent with the edit instructions in its prompt. The subagent's `Write`/`Edit` tool calls render collapsed under the Agent invocation in the parent conversation — the diff is invisible to the user (whose vault is already open in Obsidian side-by-side; in-conversation diffs are pure redundancy AND visually disrupt the conversational thread). Spike 2026-06-08 confirmed: Claude Code 2.1+ subagents inherit parent-session vault Write permissions (no extra prompt) and have their tool calls collapsed under the Agent block. This SUPERSEDES the prior "default inline" protocol, which is now stale. See [[2026-06-08-quiet-forge-vault-writes]] for the full rationale + spike result. Load `references/vault-write-protocol.md` for the dispatch pattern and the spike receipts.
  - **Inline Write/Edit only as fallback** — when subagent dispatch is impractical for a specific case (e.g. a 2-line frontmatter timestamp bump right after a context compression event where the subagent overhead isn't worth the noise saved). Inline produces visible diff render; minimize when used (smallest possible Edit, never re-Write the whole file for a small change).
  - **Batch multiple vault edits into ONE subagent dispatch.** Multiple files in a single prompt is strictly better than multiple `Agent` calls — the subagent does all the writes; the parent conversation sees one collapsed Agent block instead of N. Background dispatch (`run_in_background: true`) remains opt-in and requires user authorization (its permission flow still has no UI surface for prompts).
