...
- If the marker is missing, empty, whitespace-only, or contains the literal `__pending__`: respond with `"Keeper is part of Forge. Want me to enter Forge mode? Say /forge to activate."` and stop. No further tool calls.
- If the marker contains valid JSON with a `project` field (the active project): prefix your output with `[Keeper]` and proceed with the dispatched task.
