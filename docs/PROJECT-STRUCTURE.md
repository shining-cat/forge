...
- BACKLOG Effort/Impact/Status cells are rendered glyph cells, never hand-typed: Effort 🟦 (XS shown as a single 🔹) / Impact 🟪 as a three-slot lane (filled boxes left, empty = centered dot `·`, letter beneath; effort scale XS·S·M·L, impact scale S·M·L·? where `?` = unknown, rendered as three empty slots); Status 🟢 active · 🟠 next · ⚪ open · 🔴 blocked · ⏳ parked · 💡 shaping. `forge-context.sh render-backlog-cell` is the single source of truth (+ the status-alias collapse to 6 buckets). The lane uses small inline-HTML spans (Obsidian polish; emoji content degrades gracefully off-Obsidian); every glyph is single-codepoint default-emoji (Unicode ≤ 12.0) so it renders identically on GitHub, Obsidian macOS + mobile, light + dark. See keeper.md Duty 5.
...
**Multi-repo:** the walk covers the outer vault repo *and* every nested private repo listed in `VAULT_PRIVATE_ROOTS` (forge.conf) — e.g. `PRO/`, gitignored by the outer repo and carrying its own remote (Schibsted GHEC). Each nested repo is shown under a `### Nested repo: <name> ###` section and, in commit mode, pushed to its own configured origin. Entries that aren't git repos (e.g. an empty `SCHIBSTED/`) are skipped silently.

**Transient artifacts:** install-generated files matching `*.upstream.*` (e.g. `_templates/draft.md.upstream.<ts>`) are excluded from the commit set — they're regenerated by `install.sh` and left unstaged.

**Two modes:**

- **Report (default):** read-only. Useful for orientation: "what's dirty in the vault and what would I commit?"
- **Commit (`--commit` flag):** walks each group, asks Y/N to commit, runs the commits, then asks whether to push. Skipped groups stay unstaged for you to handle later.
  - **With a TTY** (run in your own terminal, e.g. via `!`): prompts are interactive — per-group Y/N and a push confirmation.
  - **Without a TTY** (invoked by Claude via the Bash tool, or with `FORGE_ASSUME_DEFAULTS=1`): the prompts fall back to their defaults (auto-Y), so every group is committed and each repo pushed unattended. This is intentional — invoking `--commit` *is* the intent.

```bash
# Report
bash ~/.claude/scripts/forge-context.sh vault-sync

# Commit + push (interactive with a tty, auto-Y without one)
bash ~/.claude/scripts/forge-context.sh vault-sync --commit
```

**Refusal cases (per repo):** not under git, clean, or pre-staged files already exist (commit or unstage them first — vault-sync owns the staging area only when it's empty). A refusal on one repo doesn't abort the others.
