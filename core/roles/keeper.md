...
**Color-coded judgment columns.** Effort/Impact/Status cells are rendered glyph cells — never hand-typed. Use `forge-context.sh render-backlog-cell <effort|impact|status> <value>`, or pass `--effort`/`--impact`/`--status` to `update-backlog-row` (edit existing row) or `add-backlog-row` (insert a new row under a section, dup-guarded); the renderer is the single source of truth for the glyph scheme and the status-alias collapse to 6 buckets. Effort 🟦 and Impact 🟪 each render as a fixed three-slot lane — filled boxes left-anchored by level, empty slots shown as a centered middle-dot `·` — with the letter beneath. Effort scale is S·M·L; impact scale is S·M·L·? (`?` = unknown impact, rendered as three empty slots). Status is a dot over the word: 🟢 active · 🟠 next · ⚪ open · 🔴 blocked · ⏳ parked · 💡 shaping. The Status glance column collapses granular task statuses (underway/partial/in-progress/phase-N-only-remaining→active; needs-triage→open; dormant·low/fuzzy→blocked; deferred/scheduled/parked-until-\<date\>/parked-indefinitely→parked; idea/refine/needs-refinement→shaping); the precise status stays in each task's frontmatter. Mapping is **strict** — an unmapped status/level hard-fails (`exit 2`), no silent fallback. The lane uses small inline-HTML spans (fixed-width slots, slightly reduced glyph size) — this is Obsidian-rendered polish; the emoji *content* still degrades gracefully on GitHub/plain markdown (boxes show, the lane layout doesn't). Every glyph is single-codepoint default-emoji (Unicode ≤ 12.0), so it renders identically on GitHub, Obsidian macOS + mobile, and on both light and dark backgrounds.

**Removing a shipped row.** When a task ships, its active Hot-table row is *removed* (not struck through) so the list visibly shrinks. Use `forge-context.sh remove-backlog-row --task <slug>` — a Tier-1 path that deletes the matching active row and leaves the `<details>` history block untouched. Pair it with `add-recently-shipped` to preserve the entry in the Recently-shipped block, then `bump-backlog-header` to refresh the count. Never hand-delete the row via inline Edit.
