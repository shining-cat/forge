...
Caller (Refiner or operator) should still call `forge-context.sh append-friction --pattern needs_new_pattern --action-ref <stub>`. The stub task pre-fills "candidate for new catalog entry". Toolsmith reviews these during catalog reviews and either extends an existing pattern or adds a new one to `script-replacement-patterns.md` (then updates this tree to route to it).
